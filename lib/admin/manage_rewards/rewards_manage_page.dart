...
class _RewardsManagePageState extends State<RewardsManagePage> {
  ...
  Future<void> _showRewardForm({Map<String, dynamic>? existing}) async {
    ...
    String selectedType = existing?['type'] ?? 'VOUCHER'; bool isSaving = false;
                          ...
                          child: ElevatedButton(
                            onPressed: isSaving ? null : () async {
                              ...
                              try {
                                ...
                                final data = {'name': nameCtrl.text.trim(), 'description': descCtrl.text.trim(), 'type': selectedType, 'points_required': int.parse(pointsCtrl.text.trim()), 'stock': int.tryParse(stockCtrl.text.trim()) ?? 100, 'image_url': finalImageUrl, 'terms_condition': termsCtrl.text.trim().isEmpty ? null : termsCtrl.text.trim(), 'is_active': true};
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        backgroundColor: const Color(0xFFB71C1C), elevation: 4,
              ...
              : RefreshIndicator(
                  ...
                  child: isDesktop
                      ...
                      ? GridView.builder(
                          ...
                          gridDelegate: const SliverGridDelegateWithMaxCrossAxisExtent(maxCrossAxisExtent: 350, crossAxisSpacing: 20, mainAxisSpacing: 20, mainAxisExtent: 310),
        ...
        child: Column(children: [
          if (r['image_url'] != null && r['image_url'].toString().isNotEmpty)
            ClipRRect(
              ...
              child: Image.network(r['image_url'], height: 120, width: double.infinity, fit: BoxFit.cover, errorBuilder: (_, __, ___) => Container(height: 60, color: const Color(0xFFF3F4F6), child: const Center(child: Icon(Icons.broken_image_rounded, color: Color(0xFFD1D5DB))))),
            ),
          Expanded(
            child: Padding(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                const Spacer(),
                Row(children: [
                  _actionBtn(Icons.edit_rounded, 'Edit', const Color(0xFF3B82F6), () => _showRewardForm(existing: r)), const SizedBox(width: 8),
                  _actionBtn(isActive ? Icons.visibility_off_rounded : Icons.visibility_rounded, isActive ? 'Nonaktifkan' : 'Aktifkan', const Color(0xFFF59E0B), () => _toggleActive(r)), const SizedBox(width: 8),
  ...
  Widget _actionBtn(IconData icon, String label, Color color, VoidCallback onTap) {
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Row(mainAxisSize: MainAxisSize.min, children: [Icon(icon, size: 14, color: color), const SizedBox(width: 4), Flexible(child: Text(label, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: color), overflow: TextOverflow.ellipsis))]),
      ),
    );
  }
}