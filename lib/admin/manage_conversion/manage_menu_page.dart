...
import 'accurate_sync_history_page.dart';
import 'voucher_manage_page.dart'; // [BARU]
...
class ManageMenuPage extends StatelessWidget {
    ...
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Wrap(spacing: 16, runSpacing: 0, children: [
          ...
          _menuWrapper(isDesktop, _MenuItem(icon: Icons.card_giftcard_rounded, color: const Color(0xFFB71C1C), title: 'Kelola Hadiah', subtitle: 'Tambah, edit, hapus hadiah & best deal', delay: 80, onTap: () => Navigator.push(context, _slideRoute(const RewardsManagePage())))),
          ...
          // [BARU] Kelola Penukaran Voucher
          _menuWrapper(isDesktop, _MenuItem(icon: Icons.swap_horiz_rounded, color: const Color(0xFF10B981), title: 'Kelola Penukaran', subtitle: 'Verifikasi & ubah status klaim hadiah', delay: 280, isNew: true, onTap: () => Navigator.push(context, _slideRoute(const VoucherManagePage())))),
        ...
        Wrap(spacing: 16, runSpacing: 0, children: [
          _menuWrapper(isDesktop, _MenuItem(icon: Icons.person_add_alt_1_rounded, color: const Color(0xFF7C3AED), title: 'Daftarkan User', subtitle: 'Register toko kecil oleh admin', delay: 360, onTap: () => Navigator.push(context, _slideRoute(const AdminRegisterUserPage())))),
          _menuWrapper(isDesktop, _MenuItem(icon: Icons.person_search_rounded, color: const Color(0xFF10B981), title: 'Pelanggan Accurate', subtitle: 'Cek toko yang belum terdaftar', delay: 380, onTap: () => Navigator.push(context, _slideRoute(const UnregisteredCustomersPage())))),
        ]),
        ...
        Wrap(spacing: 16, runSpacing: 0, children: [
          ...
          _menuWrapper(isDesktop, _MenuItem(icon: Icons.manage_history_rounded, color: const Color(0xFF3B82F6), title: 'Sync & Histori Poin', subtitle: 'Tarik poin faktur & retur otomatis', delay: 440, onTap: () => Navigator.push(context, _slideRoute(const _SyncHistoryWrapper())))),
        ]),
      ]),
    );
  }
  ...
  Widget _sectionDivider(String label) {
    return Container(margin: const EdgeInsets.only(bottom: 16), child: Row(children: [
      Container(width: 40, height: 1, color: const Color(0xFFE5E7EB)), const SizedBox(width: 8),
      Text(label, style: const TextStyle(fontSize: 12, color: Color(0xFF9CA3AF), fontWeight: FontWeight.w600)),
      const SizedBox(width: 8), Expanded(child: Container(height: 1, color: const Color(0xFFE5E7EB))),
    ]));
  }
...
class _AccurateWrapper extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) { return Scaffold(backgroundColor: const Color(0xFFF8F8FB), appBar: AppBar(backgroundColor: Colors.white, elevation: 0, leading: IconButton(onPressed: () => Navigator.pop(context), icon: const Icon(Icons.arrow_back_rounded, color: Color(0xFF1A1A2E))), title: const Text('Koneksi Accurate', style: TextStyle(color: Color(0xFF1A1A2E), fontWeight: FontWeight.w700, fontSize: 18))), body: const AccurateConnectPage()); }
}
