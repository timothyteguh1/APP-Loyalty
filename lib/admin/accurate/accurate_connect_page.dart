...
class _AccurateConnectPageState extends State<AccurateConnectPage> {
  ...
  Future<void> _init() async {
    ...
    await _loadConfig();
    if (kIsWeb && !_isConnected) _tryReadTokenFromUrl();
  }
  ...
  void _tryReadTokenFromUrl() {
    try {
      ...
      String tokenPart = '';
      if (href.contains('#')) { tokenPart = href.split('#').last; } else if (href.contains('?')) { tokenPart = href.split('?').last; }
      if (tokenPart.isEmpty) return;
      final params = Uri.splitQueryString(tokenPart);
      final token = params['access_token'];
      if (token != null && token.isNotEmpty) {
        WidgetsBinding.instance.addPostFrameCallback((_) async { await _saveTokenAndOpenDb(token); });
      }
    } catch (e) { debugPrint('URL token read error: $e'); }
  }
  ...
  Future<void> _submitManualToken() async {
    ...
    if (token.isEmpty) { _showSnack('Token tidak boleh kosong', false); return; }
  ...
  Future<void> _connectAccurate() async {
    ...
    if (await canLaunchUrl(uri)) {
      ...
      if (mounted) { await Future.delayed(const Duration(seconds: 2)); if (mounted) setState(() => _showManualToken = true); }
    } else { _showSnack('Tidak bisa membuka browser', false); }
  }
  ...
  Future<void> _startSync() async {
    ...
    try {
      ...
      else { _showSnack('Sync gagal: $e', false); }
  ...
  Future<void> _disconnect() async {
    final confirm = await showDialog<bool>(context: context, builder: (ctx) => AlertDialog(
      ...
      actions: [TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text('Batal')), ElevatedButton(onPressed: () => Navigator.pop(ctx, true), style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFFEF4444), elevation: 0), child: const Text('Putuskan', style: TextStyle(color: Colors.white)))],
    ));
  ...
  bool get _isSessionOpen => (_config['accurate_db_session'] ?? '').isNotEmpty && (_config['accurate_db_host'] ?? '').isNotEmpty;
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              _buildCard(step: 1, title: 'Hubungkan Akun Accurate', subtitle: _isConnected ? 'Akun Accurate sudah terhubung ✓' : 'Login ke Accurate lalu paste token', isDone: _isConnected, child: Column(children: [
                if (!_isConnected) ...[
                  ...
                  SizedBox(width: double.infinity, height: 44, child: OutlinedButton.icon(onPressed: () => setState(() => _showManualToken = !_showManualToken), icon: const Icon(Icons.vpn_key_rounded, size: 16), label: const Text('2. Paste Access Token di sini', style: TextStyle(fontWeight: FontWeight.w600)), style: OutlinedButton.styleFrom(foregroundColor: const Color(0xFF059669), side: const BorderSide(color: Color(0xFF059669)), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))))),
                  if (_showManualToken) ...[
                    const SizedBox(height: 12), _buildTokenInstructions(), const SizedBox(height: 10),
                    TextField(controller: _tokenController, maxLines: 3, style: const TextStyle(fontSize: 12, fontFamily: 'monospace'), decoration: InputDecoration(hintText: 'Paste access_token dari URL setelah login Accurate...', hintStyle: TextStyle(color: Colors.grey[400], fontSize: 12), filled: true, fillColor: const Color(0xFFF8F9FC), contentPadding: const EdgeInsets.all(12), border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: Colors.grey[200]!)), enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: Colors.grey[200]!)), focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: const BorderSide(color: Color(0xFF059669), width: 1.5)))),
                    const SizedBox(height: 8),
                    SizedBox(width: double.infinity, height: 44, child: ElevatedButton(onPressed: _submitManualToken, style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF059669), foregroundColor: Colors.white, elevation: 0, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))), child: const Text('Simpan Token', style: TextStyle(fontWeight: FontWeight.w600)))),
                  ],
                ] else ...[
                  Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: const Color(0xFFF0FDF4), borderRadius: BorderRadius.circular(10), border: Border.all(color: const Color(0xFFBBF7D0))), child: Row(children: [const Icon(Icons.verified_rounded, color: Color(0xFF10B981), size: 20), const SizedBox(width: 10), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('Token aktif', style: TextStyle(fontWeight: FontWeight.w700, color: Color(0xFF065F46), fontSize: 13)), Text('Expire: ${_config['accurate_token_expiry']?.substring(0, 10) ?? '-'}', style: const TextStyle(fontSize: 11, color: Color(0xFF6B7280)))])), TextButton(onPressed: _disconnect, child: const Text('Putuskan', style: TextStyle(color: Color(0xFFEF4444), fontSize: 12)))])),
                ],
              ])),
  ...
  Widget _buildTokenInstructions() {
    return Container(padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: const Color(0xFFF0F9FF), borderRadius: BorderRadius.circular(12), border: Border.all(color: const Color(0xFFBAE6FD))),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Container(margin: const EdgeInsets.only(top: 4), padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(8)), child: const SelectableText('localhost:3000/#access_token=abc123...&token_type=bearer', style: TextStyle(fontSize: 10, fontFamily: 'monospace', color: Color(0xFF059669)))),
      ]));
  }
  ...
  Widget _instrStep(String n, String text) {
    return Padding(padding: const EdgeInsets.only(bottom: 4), child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Container(width: 18, height: 18, margin: const EdgeInsets.only(top: 1), decoration: const BoxDecoration(color: Color(0xFF3B82F6), shape: BoxShape.circle), child: Center(child: Text(n, style: const TextStyle(fontSize: 10, color: Colors.white, fontWeight: FontWeight.w700)))),
  ...
  Widget _buildStatusCard() {
    Color color; String status; String subtitle; IconData icon;
    if (_isSessionOpen) { color = const Color(0xFF10B981); status = 'Terhubung & Siap Sync'; subtitle = 'Database Accurate aktif'; icon = Icons.check_circle_rounded; }
    else if (_isConnected) { color = const Color(0xFFF59E0B); status = 'Token Valid — Database Belum Dibuka'; subtitle = 'Klik "Buka Database" di bawah'; icon = Icons.pending_rounded; }
    else { color = const Color(0xFF9CA3AF); status = 'Belum Terhubung'; subtitle = 'Ikuti langkah di bawah'; icon = Icons.link_off_rounded; }
    return Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: color.withOpacity(0.08), borderRadius: BorderRadius.circular(16), border: Border.all(color: color.withOpacity(0.3))), child: Row(children: [Icon(icon, color: color, size: 32), const SizedBox(width: 14), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(status, style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: color)), const SizedBox(height: 2), Text(subtitle, style: const TextStyle(fontSize: 12, color: Color(0xFF6B7280)))]))]));
  }
  ...
  Widget _buildCard({required int step, required String title, required String subtitle, required Widget child, bool isDone = false, bool isDisabled = false}) {
    return Opacity(opacity: isDisabled ? 0.45 : 1.0, child: Container(padding: const EdgeInsets.all(18), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(16), border: Border.all(color: isDone ? const Color(0xFF10B981).withOpacity(0.3) : const Color(0xFFF0F0F0))),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [Container(width: 28, height: 28, decoration: BoxDecoration(color: isDone ? const Color(0xFF10B981) : const Color(0xFFB71C1C), shape: BoxShape.circle), child: Center(child: isDone ? const Icon(Icons.check_rounded, color: Colors.white, size: 16) : Text('$step', style: const TextStyle(color: Colors.white, fontSize: 13, fontWeight: FontWeight.w700)))), const SizedBox(width: 12), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: Color(0xFF1A1A2E))), Text(subtitle, style: const TextStyle(fontSize: 12, color: Color(0xFF6B7280)))]))]),
        if (!isDisabled) ...[const SizedBox(height: 16), child],
      ])));
  }

  Widget _buildSyncResult(SyncResult result) {
    return Container(padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: const Color(0xFFF0FDF4), borderRadius: BorderRadius.circular(12), border: Border.all(color: const Color(0xFFBBF7D0))),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        if (result.errors.isNotEmpty) ...[const SizedBox(height: 6), ...result.errors.take(3).map((e) => Text('⚠ $e', style: const TextStyle(fontSize: 11, color: Color(0xFFEF4444))))],
      ]));
  }

  Widget _resultRow(String label, String value) {
    return Padding(padding: const EdgeInsets.only(bottom: 4), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text(label, style: const TextStyle(fontSize: 12, color: Color(0xFF6B7280))), Text(value, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: Color(0xFF065F46)))]));
  }

  Widget _buildGuideCard() {
    return Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(16), border: Border.all(color: const Color(0xFFF0F0F0))),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Row(children: [Icon(Icons.person_search_rounded, size: 18, color: Color(0xFF3B82F6)), SizedBox(width: 8), Text('Setup Accurate ID per Toko', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700))]),
        const SizedBox(height: 10),
