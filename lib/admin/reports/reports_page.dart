...
class _ReportsPageState extends State<ReportsPage> with SingleTickerProviderStateMixin {
    ...
    try {
      ...
      if (mounted) {
        ...
        for (var item in data) {
          if ((item['amount'] as num? ?? 0) < 0) items.add(item['description'] ?? 'Lainnya');
        }
        setState(() {
          _historyData = data;
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Gagal memuat data: $e'), backgroundColor: const Color(0xFFEF4444)));
      }
    }
  }
  ...
  Future<void> _pickDateRange(bool isIncome) async {
    ...
    final result = await showDateRangePicker(
      ...
      lastDate: now,
      initialDateRange:
          initial ?? DateTimeRange(start: now.subtract(const Duration(days: 30)), end: now),
      builder: (context, child) => Theme(
        data: Theme.of(context).copyWith(
            colorScheme: const ColorScheme.light(
                primary: Color(0xFFB71C1C), onPrimary: Colors.white, surface: Colors.white)),
        child: child!,
      ),
    );
    if (result != null) {
      setState(() {
        if (isIncome) {
          ...
          _dateRangeOut = result;
        }
      });
    }
  }

  // [FIX] Bypass RPC — langsung insert ke point_history dengan deskripsi unik per user
  // Root cause: kolom description di point_history punya UNIQUE constraint,
  // RPC lama pakai deskripsi yang sama untuk semua user sehingga bentrok di baris ke-2.
  Future<void> _resetAnnualPoints() async {
    ...
    try {
      ...
      // [FIX] Cek apakah reset tahun ini sudah pernah dilakukan
      final checkExisting = await _admin
          .from('point_history')
          .select('id')
          .eq('reference_type', 'SYSTEM_CUTOFF')
          .ilike('description', '%Cut Off Poin Tahunan $year%')
          .limit(1);

      if ((checkExisting as List).isNotEmpty) {
        setState(() => _isLoading = false);
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text('Reset tahun $year sudah pernah dilakukan sebelumnya.'),
      ...
      // Ambil semua user APPROVED
      final users = await _admin
          .from('profiles')
      ...
      for (final user in users) {
        ...
        // [FIX] Deskripsi unik per user per tahun — pakai 8 karakter pertama UUID
        final String uniqueDesc =
            'Cut Off Poin Tahunan $year [${userId.substring(0, 8)}]';

        // Insert catatan pemotongan poin ke history
        await _admin.from('point_history').insert({
          'user_id': userId,
          'amount': -currentPoints,
          'description': uniqueDesc,
          'reference_type': 'SYSTEM_CUTOFF',
          'created_at': DateTime.now().toIso8601String(),
        });

        // Set poin profil ke 0
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text('Reset berhasil! $resetCount toko telah direset ke 0 poin.'),
        ...
        child: ConstrainedBox(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Padding(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                const Text('Laporan & Analytics',
                    style: TextStyle(
                        color: Color(0xFF1A1A2E), fontWeight: FontWeight.w800, fontSize: 24)),
                const SizedBox(height: 4),
                const Text('Rekapitulasi pendapatan dan penukaran poin.',
            ...
            Expanded(
              child: _isLoading
                  ...
                  : TabBarView(
                      controller: _tabController,
                      children: [_buildDashboardTab(), _buildIncomeTab(), _buildRedeemTab()],
                    ),
            ),
          ]),
        ),
  ...
  Widget _buildDashboardTab() {
    ...
    for (var item in _historyData) {
      ...
      if (amount > 0) {
        ...
        itemStats[desc] = (itemStats[desc] ?? 0) + 1;
      }
    }
    ...
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Expanded(
              child: _statCard(
                  'Total Poin Masuk', '+$totalIn', const Color(0xFF10B981), Icons.arrow_downward_rounded)),
          const SizedBox(width: 12),
          Expanded(
              child: _statCard('Total Poin Ditukar', '-$totalOut', const Color(0xFFEF4444),
        ...
        Container(
          ...
          child: Row(children: [
            Container(
                ...
                child: const Icon(Icons.date_range_rounded, color: Color(0xFFEF4444))),
            const SizedBox(width: 12),
            const Expanded(
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text('Tutup Buku Tahunan',
                    style: TextStyle(fontWeight: FontWeight.w700, color: Color(0xFF991B1B))),
                Text('Reset semua poin toko menjadi 0',
            ...
            ElevatedButton(
                ...
                style: ElevatedButton.styleFrom(
                    backgroundColor: const Color(0xFFEF4444), elevation: 0, foregroundColor: Colors.white),
                child: const Text('Eksekusi')),
          ]),
        ),
        const SizedBox(height: 32),
        const Text('Top Hadiah Ditukar',
        ...
        if (sortedItems.isEmpty)
          ...
          ...sortedItems.take(10).map((e) => Container(
                ...
                child: Row(children: [
                  ...
                  Text('${e.value}x',
                      style: const TextStyle(fontWeight: FontWeight.w700, color: Color(0xFFB71C1C))),
                ]),
              )),
      ]),
    );
    ...
    final incomeData = _historyData.where((item) {
      ...
      if (_dateRangeIn != null && item['created_at'] != null) {
        try {
          ...
          if (dt.isBefore(_dateRangeIn!.start) ||
              dt.isAfter(_dateRangeIn!.end.add(const Duration(days: 1)))) return false;
        } catch (_) {}
      }
      return true;
    }).toList();
    return Column(children: [
      ...
      Expanded(
          child: incomeData.isEmpty
              ? const Center(
                  child: Text('Tidak ada data pendapatan poin.', style: TextStyle(color: Colors.grey)))
              : isDesktop
                  ? _buildDesktopTable(incomeData, true)
                  : ListView.builder(
  ...
  Widget _buildRedeemTab() {
    ...
    final redeemData = _historyData.where((item) {
      ...
      if (_selectedItemFilter != null &&
          _selectedItemFilter != 'Semua Barang' &&
          desc != _selectedItemFilter) return false;
      if (_dateRangeOut != null && item['created_at'] != null) {
        try {
          final dt = DateTime.parse(item['created_at']).toLocal();
          if (dt.isBefore(_dateRangeOut!.start) ||
              dt.isAfter(_dateRangeOut!.end.add(const Duration(days: 1)))) return false;
        } catch (_) {}
      }
      return true;
    }).toList();
      ...
      _buildFilterBar(
          ...
          extraWidget: SizedBox(
              ...
              child: DropdownButtonFormField<String>(
                  ...
                  items: ['Semua Barang', ..._redeemedItems]
                      ...
                      .toList(),
                  onChanged: (v) =>
                      setState(() => _selectedItemFilter = v == 'Semua Barang' ? null : v)))),
      Expanded(
          child: redeemData.isEmpty
              ? const Center(
                  child:
                      Text('Tidak ada riwayat penukaran.', style: TextStyle(color: Colors.grey)))
              : isDesktop
                  ? _buildDesktopTable(redeemData, false)
                  : ListView.builder(
              ...
              child: Container(
                  ...
                  child: Row(mainAxisSize: MainAxisSize.min, children: [
                    Icon(Icons.date_range_rounded,
                        size: 18,
                        color: dateRange != null ? const Color(0xFFB71C1C) : Colors.grey),
                    const SizedBox(width: 6),
                    Text(
                        dateRange != null
      ...
      child: Row(children: [
        Container(
            ...
            decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.circular(12)),
            child: Icon(isIn ? Icons.arrow_downward_rounded : Icons.card_giftcard_rounded,
                color: isIn ? const Color(0xFF10B981) : const Color(0xFFEF4444), size: 20)),
