...
class _ReportsPageState extends State<ReportsPage> with SingleTickerProviderStateMixin {
  ...
  Future<void> _resetAnnualPoints() async {
    ...
    try {
      ...
      if (mounted) { setState(() => _isLoading = false); ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Gagal reset: $e'), backgroundColor: const Color(0xFFEF4444))); }
    }
  }
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        if (sortedItems.isEmpty) const Text('Belum ada data penukaran hadiah.', style: TextStyle(color: Colors.grey))
          ...
          child: Row(children: [
            ...
            const SizedBox(width: 12),
            Expanded(child: Text(e.key, style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 14), maxLines: 2, overflow: TextOverflow.ellipsis)),
            Text('${e.value}x', style: const TextStyle(fontWeight: FontWeight.w700, color: Color(0xFFB71C1C))),
          ]),
        )),
      ]),
    );
                ...
                rows: data.map((item) {
                  ...
                  return DataRow(cells: [
                    ...
                    DataCell(Container(
                      ...
                      decoration: BoxDecoration(color: (isIn ? const Color(0xFF10B981) : const Color(0xFFEF4444)).withOpacity(0.1), borderRadius: BorderRadius.circular(8)),
  ...
  Widget _historyTile(Map<String, dynamic> item, bool isIn) {
    ...
    return Container(
      ...
      child: Row(children: [
        Container(width: 44, height: 44, decoration: BoxDecoration(color: (isIn ? const Color(0xFF10B981) : const Color(0xFFEF4444)).withOpacity(0.1), borderRadius: BorderRadius.circular(12)),
