...
class _DetailRewardPageState extends State<DetailRewardPage> {
  ...
  Future<void> _claimReward() async {
    ...
    try {
      ...
      // ======= EMAIL NOTIFIKASI: Reward Claimed =======
      final user = _supabase.auth.currentUser;
      if (user?.email != null) {
        EmailNotificationService.sendRewardClaimed(
          toEmail: user!.email!,
          userName: user.userMetadata?['full_name'] ?? 'User',
          rewardName: widget.item['name'] ?? 'Hadiah',
        );
      }
      // ======= END EMAIL =======
              ...
              child: Column(mainAxisSize: MainAxisSize.min, children: [
                ...
                Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                  Row(children: [
                    ...
                    Text("${widget.item['points_required']}", style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold)),
                  ]),
                  ElevatedButton(
                    onPressed: _claimReward,
                    style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFFD32F2F), foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(horizontal: 32, vertical: 12), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
